...
from .models import School, TypeSchool, Year, ExceptionDay, ExceptionTime, TermYear, TermType
...
def get_term_year_by_id(term_year_id):
    ...
    Returns:
        TermYear: L'objet TermYear ou None si non trouvé.
    """
    try:
        return TermYear.objects.get(id=term_year_id)
    except TermYear.DoesNotExist:
        return None

def update_term_year(term_year_id, **kwargs):
    """
    ...
    try:
        ...
        return None, f"Erreur lors de la mise à jour du trimestre/semestre : {str(e)}"

def delete_term_year(term_year_id):
    """
    Supprime un objet TermYear.
    Args:
        term_year_id (int): L'ID du trimestre/semestre à supprimer.
    Returns:
        bool: True si la suppression est réussie, False sinon.
    """
    try:
...
def finish_term_year(term_year_id):
    """
    Marque un trimestre/semestre comme terminé.
    Args:
        term_year_id (int): L'ID du trimestre/semestre à marquer comme terminé.
    Returns:
        tuple: (TermYear, str) - L'objet TermYear mis à jour ou un message d'erreur.
    """
    try:
        term_year = TermYear.objects.get(id=term_year_id)
        term_year.finished = True
        term_year.save()
        return term_year, None
    except TermYear.DoesNotExist:
        return None, "Trimestre/semestre non trouvé."
